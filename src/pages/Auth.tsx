...
export default function Auth() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (!selectedTenant) {
      toast({ title: "Please select a campus", variant: "destructive" });
      return;
    }
    setLoading(true);
  ...
  const handleTenantRegistration = async (e: React.FormEvent) => {
    ...
    if (authData.user) {
      ...
      if (tenantError) {
        toast({ title: "Failed to create organization", description: tenantError.message, variant: "destructive" });
        setLoading(false);
        return;
      }
      ...
      if (tenantData) {
        ...
        toast({ title: "Organization registered!", description: "Your campus has been set up. You are now signed in as admin." });
        navigate("/dashboard");
      }
    }
    setLoading(false);
    ...
    <div className="min-h-screen flex items-center justify-center bg-background p-4">
      ...
      <Card className="w-full max-w-md relative animate-fade-up shadow-elevated">
        <CardHeader className="text-center">
          ...
          <CardTitle className="font-display text-2xl">CampusCare ERP</CardTitle>
          <CardDescription>Campus Complaint & Maintenance Management</CardDescription>
        </CardHeader>
        <CardContent>
          {tab === "register-tenant" ? (
            <form onSubmit={handleTenantRegistration} className="space-y-4">
              <Button type="button" variant="ghost" size="sm" className="mb-2" onClick={() => setTab("login")}>
                <ArrowLeft className="mr-2 h-4 w-4" /> Back to login
              </Button>

              <div className="space-y-2">
                <Label>Campus/Organization Name</Label>
                <Input placeholder="State University" value={tenantName} onChange={(e) => setTenantName(e.target.value)} required />
              </div>

              <div className="space-y-2">
                <Label>Campus Slug (unique identifier)</Label>
                <Input placeholder="state-university" value={tenantSlug} onChange={(e) => setTenantSlug(e.target.value.replace(/\s+/g, "-"))} required />
              </div>
              ...
              <div className="border-t border-border pt-4 mt-4">
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Input placeholder="John Doe" value={adminName} onChange={(e) => setAdminName(e.target.value)} required />
                  </div>
                  <div className="space-y-2">
                    <Label>Email</Label>
                    <Input type="email" placeholder="admin@university.edu" value={adminEmail} onChange={(e) => setAdminEmail(e.target.value)} required />
                  </div>
                  <div className="space-y-2">
            ...
            <Tabs value={tab} onValueChange={(v) => setTab(v as "login" | "signup")}>
              ...
              <div className="mt-6 pt-6 border-t border-border text-center">
                <p className="text-sm text-muted-foreground mb-2">New campus? Register your organization</p>
                <Button variant="outline" className="w-full" onClick={() => setTab("register-tenant")}>
                  Register New Campus
                </Button>
              </div>
            </Tabs>
